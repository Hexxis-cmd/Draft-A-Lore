...
/* --- Menu rendering (shared by the menu bar and right-click menus) --- */
...
DAL.menuItemHtml = function(it, subIndex){
  ...
  if(it.heading) return '<div class="menu-heading">'+DAL.escapeHtml(it.heading)+'</div>';
  if(it.submenu) return '<div class="menu-item has-sub" data-menu-sub="'+subIndex+'">'+DAL.escapeHtml(it.label)+'<span class="menu-chev">\u203A</span></div>';
  var cls = 'menu-item' + (it.danger ? ' danger' : '') + (it.disabled ? ' disabled' : '');
  var attrs = it.disabled ? '' : ' data-action="menu" data-do="'+DAL.escapeHtml(it.action)+'"';
...
document.addEventListener('contextmenu', function(e){
  ...
  if(!builder) return;
  var items = builder(el.getAttribute('data-ctx-id'), el, e);
  if(!items || !items.length) return;
  e.preventDefault();
  DAL.contextMenu(e.clientX, e.clientY, items);
});
