...
(function(){
  ...
  DAL.unbindAssetEverywhere = function(proj, id){
    function scrub(holder){
      ...
      ['voice','ambient'].forEach(function(slot){ if(holder.audio[slot] === id) holder.audio[slot] = ''; });
    }
  ...
  /* Two channels, because an ambient bed and a spoken line have to coexist: the
     bed loops quietly underneath while narration plays over it. */
  var Bus = DAL.audioBus = {
    ambient: null, voice: null, ambientId: null, projectId: null
  };

  function volumes(){
    var proj = DAL.state.projects[Bus.projectId] || DAL.activeProject();
    if(!proj) return { ambient:0.4, voice:1, muted:false };
    DAL.ensureAssets(proj);
    return { ambient: proj.audio.ambientVolume, voice: proj.audio.voiceVolume, muted: proj.audio.muted };
  }
  ...
  Bus.playAmbient = function(proj, assetId){
    ...
    /* Autoplay is refused until the page has been interacted with. That is not an
       error worth shouting about — the next click starts it. */
  ...
  Bus.playVoice = function(proj, assetId){
    var src = DAL.assetSrc(proj, assetId);
  ...
  Bus.stopAll = function(){ Bus.stopAmbient(); Bus.stopVoice(); };

  /* --- Narration --------------------------------------------------------- */

  /* A bound voice clip always wins; text-to-speech is the fallback so a scene is
     never silent just because the author has not recorded it yet. */
  ...
  DAL.presentScene = function(proj, holder, text){
    DAL.ensureAssets(proj);
    Bus.attach(proj.id);
    ...
    if(audio.voice){
      Bus.playVoice(proj, audio.voice).then(function(ok){
        if(!ok) TTS.speak(text, proj);
      });
  ...
  DAL.exportAssetIds = function(proj){
    ...
    function holder(h){
      if(!h) return;
      if(h.audio){ take(h.audio.ambient); take(h.audio.voice); }
  ...
  DAL.exportHasAudio = function(proj, adv){
    ...
    ((adv && adv.nodes) || []).forEach(function(n){
      if(n && n.audio && (n.audio.ambient || n.audio.voice)) found = true;
    });
    return found;
  };

  /* --- Reading a manuscript aloud ---------------------------------------- */
  ...
  DAL.startReadAloud = function(proj, holder){
    ...
    if(holder) DAL.presentScene(proj, holder, text);
