...
if not exist "%DAL_APPDIR%\index.html" (
  echo   [X] index.html was not found next to this installer.
  echo       Keep Install-DraftALore.bat inside the Draft A Lore
...
if "%DAL_DESKTOP%"=="0" if "%DAL_STARTMENU%"=="0" (
  ...
  echo   You can always open index.html directly.
  echo.
  pause
  exit /b 0
)
...
if not "%RC%"=="0" (
  ...
  echo       You can still run the app by opening index.html.
) else (
...
try {
  ...
  if ($browser) {
    ...
    # No Chromium browser found: hand index.html to the default browser.
    $targetPath = $index
    $arguments  = ''
